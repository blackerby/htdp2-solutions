...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname exercise515) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
; A Lam is one of:
; - a Symbol
  ...
  (local
    (; Lam [List-of Symbol] -> Lam
     ...
     (define (undeclareds/a le declareds)
       (cond
         [(is-var? le)
          (string->symbol (format (if (member? le declareds)
                                      "*declared:~a"
                                      "*undeclared:~a") le))]
         [(is-λ? le)
