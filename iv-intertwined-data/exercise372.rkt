...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname exercise372) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define BT (overlay/align 'middle 'middle (circle 2 "solid" "black")
                    (square 12 "solid" "transparent")))

; XItem.v1 -> Image 
; renders an item as a "word" prefixed by a bullet
(check-expect (render-item1 '(li (word ((text "one")))))
              (beside/align 'center (overlay/align 'middle 'middle (circle 2 "solid" "black")
                                                   (square 12 "solid" "transparent"))
                            (text "one" 12 'black)))
...
; the function extracts the content from the given XItem.v1
; then extracts the element from the content
; then extracts the text from the element
; then creates a text image with the text.
; the body of the function aligns the image BT next to the text image.
