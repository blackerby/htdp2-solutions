...
(define (tree-pick tree path)
  (cond
    ...
    [(and (branch? tree)
          (empty? path)) tree]
    [(and (symbol? tree)
          (not (empty? path))) (error "path exceeds tree depth")]
    [(and (branch? tree)
          (not (empty? path)))
     (cond
...
; great solution here:
; https://gitlab.com/cs-study/htdp/-/blob/main/04-Intertwined-Data/23-Simultaneous-Processing/Exercise-390.rkt
