...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname exercise353-354) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; replaces variables in ex with values in da and yields result if ex is numeric
; or produces an error
(check-expect (eval-variable* 3 al0) 3)
(check-error (eval-variable* (make-add 'x 3) al0) WRONG)
(check-expect (eval-variable* (make-add 'x 3) al1) 6)
(check-error (eval-variable* (make-add (make-mul 'x 2) (make-mul 'y 4)) al1) WRONG)
(check-expect (eval-variable* (make-add (make-mul 'x 2) (make-mul 'y 4)) al2) 14)
