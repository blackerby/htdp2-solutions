...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname exercise362) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(check-expect (parse-atom 'hello) 'hello)
(define (parse-atom s)
  (cond
    [(number? s) s]
...
; SL -> Boolean
(check-expect (consists-of-2 (list 1 2 3)) #false)
(check-expect (consists-of-2 (list 1 2 3 4)) #false)
(check-expect (consists-of-2 (list 1 2)) #true)
...
(check-expect (parse-sl '(f 1)) (make-app 'f 1))
(check-expect (parse-sl (list 'define 'close-to-pi 3.14)) (make-con 'close-to-pi 3.14))
(check-expect (parse-sl '(define (square x) (* x x))) (make-def 'square 'x (make-mul 'x 'x)))
(check-expect (parse-sl '((define close-to-pi 3.14)
                          (define (square x) (* x x))))
                        (list (make-con 'close-to-pi 3.14)
                              (make-def 'square 'x (make-mul 'x 'x))))
(define (parse-sl s)
  (cond
    [(andmap list? s)
     (map parse-sl s)]
    [(and (consists-of-2 s) (symbol? (first s)))
     (make-app (first s) (second s))]
    [(and (consists-of-3 s) (symbol? (first s)))
     (cond
       ...
       [(and (symbol=? (first s) 'define)
             (symbol? (second s)))
        (make-con (parse (second s)) (parse (third s)))]
       [(and (symbol=? (first s) 'define)
             (list? (second s)))
        (make-def (parse (first (second s)))
                  (parse (second (second s)))
                  (parse (third s)))]
       [else (error WRONG)])]
    [else (error WRONG)]))

(define da-sexp '((define close-to-pi 3.14)
...
; S-expr SL -> Number
; parses and evaluates sexp in environment da
(check-within (interpreter '(area-of-circle 1) da-sexp) 3.14 0.001)
(check-within (interpreter '(volume-of-10-cylinder 1) da-sexp) 31.40 0.001)
(check-within (interpreter '(* 3 close-to-pi) da-sexp) 9.42 0.001)
(define (interpreter sexp da)
  (eval-all (parse sexp) (parse da)))
