...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname exercise483) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
; A Board is a [List-of [List-of QP]]
; interpretation:
; a list of rows on a chess board. the absence of a QP indicates that it is threatened.

; data example: Board
(define 2-BY-2 (list (list (make-posn 0 0) (make-posn 0 1))
                     (list (make-posn 1 0) (make-posn 1 1))))

(define 3-BY-3 (list (list (make-posn 0 0) (make-posn 0 1) (make-posn 0 2))
                     (list (make-posn 1 0) (make-posn 1 1) (make-posn 1 2))
                     (list (make-posn 2 0) (make-posn 2 1) (make-posn 2 2))))

(define 4-BY-4 (list (list (make-posn 0 0) (make-posn 0 1) (make-posn 0 2) (make-posn 0 3))
                     (list (make-posn 1 0) (make-posn 1 1) (make-posn 1 2) (make-posn 1 3))
                     (list (make-posn 2 0) (make-posn 2 1) (make-posn 2 2) (make-posn 2 3))
                     (list (make-posn 3 0) (make-posn 3 1) (make-posn 3 2) (make-posn 3 3))))
...
(define (board0 n)
  (foldr (lambda (i b)
           (cons (build-list n (lambda (j) (make-posn i j))) b))
         '() (range 0 n 1)))
 
; Board QP -> Board 
; places a queen at qp on a-board
(check-expect (add-queen 2-BY-2 (make-posn 0 0)) (list '() '()))
(check-expect (add-queen 3-BY-3 (make-posn 0 0)) (list '() (list (make-posn 1 2)) (list (make-posn 2 1))))
(define (add-queen a-board qp)
  (map (lambda (row) (filter (lambda (square) (not (threatening? qp square))) row)) a-board))
 
; Board -> [List-of QP]
; finds spots where it is still safe to place a queen
(check-expect (find-open-spots (list '() (list (make-posn 1 2)) (list (make-posn 2 1))))
              (list (make-posn 1 2) (make-posn 2 1)))
(define (find-open-spots a-board)
  (foldr (lambda (row b) (append row b)) '() a-board))
