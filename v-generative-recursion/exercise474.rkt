...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname exercise474) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(define sample-graph
 (list (list 'A 'B 'E)
       (list 'B 'E 'F)
       (list 'C 'D)
          ...
          (define (find-path/list lo-Os)
            (cond
              ...
              [else (local ((define candidate
                              (find-path (first lo-Os) destination G)))
                      (cond
                        [(boolean? candidate)
...
; point of exercise is to eliminate redundant parameters -- my take is modeled after this:
; https://github.com/S8A/htdp-exercises/blob/master/ex474.rkt
